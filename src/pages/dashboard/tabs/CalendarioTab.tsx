...
import { ChevronLeft, ChevronRight, Plus, ExternalLink, Calendar, Clock, X, BookOpen, Repeat, Sparkles } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useStudents } from '@/hooks/useStudents';
import { Student } from '@/types/student.types';
import { CalendarEvent, CreateExtraClassRequest } from '@/types/schedule.types';
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        <div className={styles.detailsHeader}>
          <div className={`${styles.detailsAvatar} ${styles[`level_${event.levelCode ?? 'basic'}`]}`}>
...
const CreateClassModal: React.FC<CreateModalProps> = ({ students, onClose, onCreated }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // TODO: substituir por chamada real quando o endpoint estiver pronto
      // const result = await scheduleService.createExtraClass(payload);
      console.log('[CalendarioTab] Payload para POST /schedule/extra-class:', payload);
      ...
      const newEvent: CalendarEvent = {
        ...
        meetPlatform: selected?.meetPlatform,
        studentStatus: selected?.status ?? 'ACTIVE',
        levelCode: selected?.levelProfileCode ?? selected?.levelProfile?.code,
      };
      onCreated(newEvent);
    } catch {
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        <div className={styles.createBody}>
          ...
          <select className={styles.select} value={form.studentId} onChange={e => set('studentId', e.target.value)}>
          ...
          <div className={styles.row2}>
            ...
            <div className={styles.col}>
              ...
              <input type="time" className={styles.input} value={form.time} onChange={e => set('time', e.target.value)} />
            </div>
          </div>
          ...
          {selected?.meetLink && (
            <p className={styles.infoNote}>
              🔗 {selected.meetPlatform === 'GOOGLE_MEET' ? 'Google Meet' : selected.meetPlatform} será usado como link da reunião.
            </p>
          )}
...
export const CalendarioTab: React.FC = () => {
  ...
  const weekEvents = useMemo(() => {
    ...
    const extra = extraEvents.filter(e => weekStrs.has(e.date));
    return [...recurringEvents, ...extra];
    ...
    <div className={styles.container}>
      ...
      <div className={styles.topBar}>
        <div className={styles.navGroup}>
          <button type="button" className={styles.navBtn} onClick={() => setWeekMonday(d => { const n = new Date(d); n.setDate(d.getDate() - 7); return n; })}>
            <ChevronLeft size={15} />
          </button>
          ...
          <button type="button" className={styles.todayBtn} onClick={() => { setWeekMonday(getMondayOfWeek(new Date())); setSelectedDayIdx(null); }}>
            Hoje
          </button>
        </div>

        <div className={styles.dayPills}>
          <button type="button" className={`${styles.dayPill} ${selectedDayIdx === null ? styles.dayPillActive : ''}`} onClick={() => setSelectedDayIdx(null)}>
            Todos
          </button>
      ...
      <div className={styles.calendarOuter}>
        ...
        <div className={styles.dayHeaderRow}>
          ...
          {visibleDates.map(date => {
            ...
            return (
              <div key={toDateStr(date)} className={`${styles.dayHeader} ${isToday ? styles.dayHeaderToday : ''}`}>
              ...
              >
                ...
                {hourTicks.map(h => (
                  <React.Fragment key={h}>
                    ...
                    {h < END_HOUR && (
                      <div className={styles.halfLine} style={{ top: (h - START_HOUR) * HOUR_PX + HOUR_PX / 2 }} />
                    )}
                  </React.Fragment>
                ))}
                ...
                {events.map(ev => {
                  ...
                  const height = Math.max(ev.durationMin * MIN_PX, 28);
                  const blocked = ev.studentStatus === 'BLOCKED';

                  return (
                    <div
                      key={ev.id}
                      className={`${styles.event} ${styles[`level_${ev.levelCode ?? 'basic'}`]} ${ev.type === 'EXTRA' ? styles.eventExtra : ''} ${blocked ? styles.eventBlocked : ''}`}
                      style={{ top, height }}
                      onClick={() => !blocked && setDetailsEvent(ev)}
                      title={blocked ? `${ev.studentName} — Bloqueado` : ev.studentName}
                    >
                      <div className={styles.eventInner}>
                        <span className={styles.eventTime}>{ev.startTime.substring(0, 5)}</span>
                        <span className={styles.eventName}>{ev.studentName}</span>
                        {ev.type === 'EXTRA' && (
                          <span className={styles.extraBadge} title="Aula avulsa">
                            <Sparkles size={9} />
                          </span>
                        )}
                      </div>
      ...
      {detailsEvent && (
        <EventDetailsModal
          ...
          onWorkspace={id => { setDetailsEvent(null); navigate(`/dashboard/student/${id}/workspace`); }}
        />
      )}
      ...
      {showCreate && (
        <CreateClassModal
          ...
          onCreated={ev => { setExtraEvents(p => [...p, ev]); setShowCreate(false); }}
        />
      )}
    </div>
  );
